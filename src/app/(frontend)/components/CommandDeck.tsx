...
export function CommandDeck({ isOpen, onClose, onOpenAuthModal }: CommandDeckProps) {
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent
        side="top"
        className="command-deck bg-black/80 backdrop-blur-xl flex flex-col justify-center items-center h-full"
      >
        <SheetHeader>
          <SheetTitle className="sr-only">Navigation Menu</SheetTitle>
        </SheetHeader>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 p-8 max-w-7xl mx-auto text-center flex-grow flex-shrink-0">
          <Link
            href="/blog"
            className="nav-card bg-white/5 border border-white/10 rounded-xl p-8 w-[90%] max-w-[280px] lg:h-[40%]"
            onClick={onClose}
          >
          ...
          <button
            ...
            className="nav-card text-left bg-white/5 border border-white/10 rounded-xl p-8 w-[90%] max-w-[280px] lg:h-[40%]"
            onClick={() => {
              onOpenAuthModal?.()
