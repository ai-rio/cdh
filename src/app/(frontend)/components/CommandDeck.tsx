...
export function CommandDeck({ isOpen, onClose }: CommandDeckProps) {
  ...
  return (
    <div id="command-deck" className={isOpen ? "open" : ""} role="dialog">
      <button
      ...
      <div
        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 p-8 max-w-7xl mx-auto text-center"
      >
        <Link href="/blog" className="nav-card" onClick={onClose}>
          <h3 className="text-xl font-bold mb-2">Blog</h3>
          <p className="text-gray-300 text-sm">
            Insights and strategies from the forefront of the creator economy.
          </p>
        </Link>
        <Link href="/pricing" className="nav-card" onClick={onClose}>
          <h3 className="text-xl font-bold mb-2">Pricing</h3>
          <p className="text-gray-300 text-sm">
            Simple, transparent plans that scale with your success.
          </p>
        </Link>
        <Link href="/about" className="nav-card" onClick={onClose}>
          <h3 className="text-xl font-bold mb-2">About Us</h3>
          <p className="text-gray-300 text-sm">
            Learn about our mission to empower professional creators.
          </p>
        </Link>
        <button id="auth-modal-trigger" className="nav-card text-left" onClick={onClose}>
