...
const ParticleCanvas = () => {
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      particles = []
      const numParticles = window.innerWidth > 768 ? 300 : 90
      for (let i = 0; i < numParticles; i++) {
        ...
        const velocity = {
          x: (Math.random() - 0.5) * 0.6, // Increased velocity
          y: (Math.random() - 0.5) * 0.6, // Increased velocity
        }
    ...
    const connect = () => {
      ...
      for (let a = 0; a < particles.length; a++) {
        for (let b = a; b < particles.length; b++) {
          ...
          if (distance < (canvas.width / 4) * (canvas.height / 4)) { // Increased connection distance
            opacityValue = 1 - (distance / 60000); // Adjusted for new distance
  ...
  return (
    <canvas
      ref={canvasRef}
      style={{ position: 'fixed', top: 0, left: 0, zIndex: -1, background: '#111111' }}
    />
  )
}
